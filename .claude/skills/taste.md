<!--
  ...
  Vendored verbatim below. Use this skill to improve UI consistency, component
  composition, design quality, code structure, project organization, naming
  quality, and UX decisions.

  Tutora design constraints that OVERRIDE any conflicting default below:
    • NO gradients anywhere (see "THE LILA RULE" — we go further: zero gradients).
    • Minimal, premium, trustworthy. Soft colors. Low-elevation shadows.
    • Font: Plus Jakarta Sans (fallback Inter). Follow the Tutora type scale.
    • Primary palette locked to Tutora tokens (Primary #4F46E5). One accent, locked.
    • Mobile app is React Native + Expo (not web) — treat web-only rules
      (Tailwind v4 postcss, next/font, backdrop-filter) as analogies, and apply
      the underlying design principle, not the literal web API.
  See .claude/context/ui-guidelines.md for the full Tutora UI/UX guide.
-->
